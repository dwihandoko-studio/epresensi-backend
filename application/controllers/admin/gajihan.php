...
class Gajihan extends CI_Controller {
	...
	public function wizard(){
        ...
        $data['v_content'] = 'member/gajihan/wizard';
    ...
    public function nextWizard(){
        ...
        $data['listData'] = $this->m_karyawan->getAllKaryawanByJabatan($this->input->post('spinJabatan'));
        $data['spinBulan'] = $this->input->post('spinPeriodeBulan');
        $data['spinTahun'] = $this->input->post('spinPeriodeTahun');
        $data['v_content'] = 'member/gajihan/listingStep2';
        $this->load->view('member/layout', $data);
    }
