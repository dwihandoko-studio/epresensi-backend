...
class m_karyawan extends CI_Model {
    ...
    function getGajihan($jabatan="all",$periodeBulan,$periodeTahun){

        $this->db->select('TG.*,TK.NamaKaryawan,TK.NIK,TK.GajiUtama,TK.NoRek,TK.NamaBank,TK.JabatanID,TK.GolonganPajakID,TJ.NamaJabatan,TJ.TarifLembur,TGP.NamaGolongan,TGP.NominalPKP');
	...
	function getRecentGajihan($limit=10){
        ...
        $this->db->join('tb_jabatan TJ','TK.JabatanID = TJ.JabatanID');
        $this->db->join('tb_golongan_pkp TGP','TK.GolonganPajakID = TGP.GolonganPajakID');
        $this->db->order_by('TG.TanggalTransfer','DESC');
		$this->db->limit($limit);
        $que = $this->db->get()->result_array();
        return $que;
